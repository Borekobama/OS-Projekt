...
public class ParallelComputation {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (isCoordinator) {
            new ParallelComputation(ownIP, ownPort).start();
        } else {
            if (args.length < 5) {
            ...
            new ParallelComputation(ownIP, ownPort, coordinatorIP, coordinatorPort).start();
        }
    }
    ...
    private void runAsWorker() throws Exception {
        ...
        try {
            ...
            // Wait for shutdown
            if (!"SHUTDOWN".equals(command)) {
                String shutdownCmd = comm.receiveBroadcast();
                System.out.println("[Worker " + rank + "] Received: " + shutdownCmd);
            }
